...
Vector3 SensorFusionEkf::ComputeInnovation(const Rotation &pose) {
    ...
    const Rotation rotation = Rotation::RotateInto(predicted_down_direction, accelerometer_measurement_);
    Vector3 axis;
    double angle;
...
void SensorFusionEkf::ComputeMeasurementJacobian() {
    for (int dof = 0; dof < 3; dof++) {
        ...
        const Vector3 delta_rotation = ComputeInnovation(epsilon_rotation * current_state_.sensor_from_start_rotation);
...
void SensorFusionEkf::ProcessAccelerometerSample(const AccelerometerData &sample) {
    ...
    if (!is_aligned_with_gravity_) {
        ...
        current_state_.sensor_from_start_rotation = Rotation::RotateInto(kCanonicalZDirection, accelerometer_measurement_);
    ...
    // S = H * P * H' + R
    innovation_covariance_ = accelerometer_measurement_jacobian_ * state_covariance_ * Transpose(accelerometer_measurement_jacobian_) + accelerometer_measurement_covariance_;

    // K = P * H' * S^-1
    kalman_gain_ = state_covariance_ * Transpose(accelerometer_measurement_jacobian_) * Inverse(innovation_covariance_);

    // x_update = K*nu
    state_update_ = kalman_gain_ * innovation_;

    // P = (I - K * H) * P;
    state_covariance_ = (Matrix3x3::Identity() - kalman_gain_ * accelerometer_measurement_jacobian_) * state_covariance_;
...
void SensorFusionEkf::UpdateMeasurementCovariance() {
    ...
    accelerometer_measurement_covariance_ = Matrix3x3::Identity() * accelerometer_noise_sigma * accelerometer_noise_sigma;
}
