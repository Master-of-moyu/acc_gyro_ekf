...
void GyroscopeBiasEstimator::ProcessGyroscope(const Vector3& gyroscope_sample,
    ...
    gyroscope_static_counter_->AppendFrame(Length(smoothed_gyroscope_delta) < kGyroscopeDeltaStaticThreshold);
...
void GyroscopeBiasEstimator::ProcessAccelerometer(const Vector3& accelerometer_sample, uint64_t timestamp_ns) {
    ...
    accelerometer_static_counter_->AppendFrame(Length(smoothed_accelerometer_delta) < kAccelerometerDeltaStaticThreshold);
...
bool GyroscopeBiasEstimator::UpdateGyroscopeBias(const Vector3& gyroscope_sample, uint64_t timestamp_ns) {
    ...
    float update_weight = std::max(0.0f, 1.0f - gyroscope_sample_norm2 / kGyroscopeForBiasThreshold);
...
bool GyroscopeBiasEstimator::IsCurrentEstimateValid() const {
    ...
    const auto off_gravity_gyro_bias = gyro_bias_lowpass - current_gravity_dir * Dot(gyro_bias_lowpass, current_gravity_dir);
    ...
    const bool isGyroscopeBiasCorrelatedWithSimulatedGyro =
        (Length(gyro_from_accel) * kRatioBetweenGyroBiasAndAccel > (Length(off_gravity_gyro_bias) + kEpsilon));
