...
class GyroscopeBiasEstimator {
  public:
    ...
    //     invocations.
    virtual void ProcessGyroscope(const vector<3>& gyroscope_sample, uint64_t timestamp_ns);
    ...
    virtual void ProcessAccelerometer(const vector<3>& accelerometer_sample, uint64_t timestamp_ns);
    ...
    virtual vector<3> GetGyroscopeBias() const;
  ...
  private:
    ...
    bool UpdateGyroscopeBias(const vector<3>& gyroscope_sample, uint64_t timestamp_ns);
    ...
    vector<3> ComputeAngularVelocityFromLatestAccelerometer(double timestep) const;
    ...
    double current_accumulated_weights_gyroscope_bias_;
    ...
    vector<3> last_mean_filtered_accelerometer_value_;
};
