...
class SensorFusionEkf {
  public:
    ...
    vector<3> GetGyroscopeBias() const {
        std::unique_lock<std::mutex> lock(mutex_);
        return {gyroscope_bias_estimate_[0],
                gyroscope_bias_estimate_[1],
  ...
  private:
    ...
    void UpdateStateCovariance(const matrix<3>& motion_update);
    ...
    vector<3> ComputeInnovation(const quaternion& pose);
    ...
    vector<3> gyroscope_bias_estimate_;
