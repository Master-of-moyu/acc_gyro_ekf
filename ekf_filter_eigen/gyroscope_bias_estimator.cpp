...
namespace {
    ...
    constexpr double kAccelerometerLowPassCutOffFrequencyHz = 1.0;
    constexpr double kRotationVelocityBasedAccelerometerLowPassCutOffFrequencyHz = 0.15;
    constexpr double kGyroscopeLowPassCutOffFrequencyHz = 1.0;
    constexpr double kGyroscopeBiasLowPassCutOffFrequencyHz = 0.15;
    ...
    constexpr double kGyroscopeForBiasThreshold = 0.30;

    // Used to monitor if accelerometer and gyroscope have been static for a few
    // frames.
    constexpr int kStaticFrameDetectionThreshold = 50;

    // Minimum time step between sensor updates.
    constexpr double kMinTimestep = 1.0; // std::chrono::nanoseconds(1);
...
GyroscopeBiasEstimator::GyroscopeBiasEstimator() :
    ...
    accelerometer_static_counter_(
        std::make_unique<IsStaticCounter>(kStaticFrameDetectionThreshold)),
...
void GyroscopeBiasEstimator::ProcessGyroscope(const vector<3>& gyroscope_sample,
    ...
    gyroscope_static_counter_->AppendFrame(smoothed_gyroscope_delta.norm() < kGyroscopeDeltaStaticThreshold);

    // Only update the bias if the gyroscope and accelerometer signals have been
    // relatively static recently.
    if (gyroscope_static_counter_->IsRecentlyStatic() && accelerometer_static_counter_->IsRecentlyStatic()) {
...
void GyroscopeBiasEstimator::ProcessAccelerometer(const vector<3>& accelerometer_sample, uint64_t timestamp_ns) {
    // Get current state of the filter.
    const uint64_t previous_accel_timestamp_ns = accelerometer_lowpass_filter_.GetMostRecentTimestampNs();
    ...
    accelerometer_static_counter_->AppendFrame(smoothed_accelerometer_delta.norm() < kAccelerometerDeltaStaticThreshold);
...
vector<3> GyroscopeBiasEstimator::ComputeAngularVelocityFromLatestAccelerometer(double timestep) const {
    ...
    // with small rotation.
    const auto incremental_rotation = quaternion::FromTwoVectors(
        vector<3>(last_mean_filtered_accelerometer_value_[0],
                  last_mean_filtered_accelerometer_value_[1],
                  last_mean_filtered_accelerometer_value_[2]),
        vector<3>(mean_of_median[0], mean_of_median[1], mean_of_median[2]));

    // We use axis angle here because this is how gyroscope values are stored.
    Eigen::AngleAxisd axis_angle(incremental_rotation);
    vector<3> incremental_rotation_axis = axis_angle.axis();
    double incremental_rotation_angle = axis_angle.angle();

    incremental_rotation_axis *= incremental_rotation_angle / timestep;

    return {static_cast<double>(incremental_rotation_axis[0]),
            static_cast<double>(incremental_rotation_axis[1]),
            static_cast<double>(incremental_rotation_axis[2])};
}

bool GyroscopeBiasEstimator::UpdateGyroscopeBias(const vector<3>& gyroscope_sample, uint64_t timestamp_ns) {
    ...
    const double gyroscope_sample_norm2 = gyroscope_sample.norm();
    if (gyroscope_sample_norm2 >= kGyroscopeForBiasThreshold) {
        return false;
    }

    double update_weight = std::max(0.0, 1.0 - gyroscope_sample_norm2 / kGyroscopeForBiasThreshold);
    update_weight *= update_weight;
    gyroscope_bias_lowpass_filter_.AddWeightedSample(gyroscope_lowpass_filter_.GetFilteredData(), timestamp_ns, update_weight);
...
vector<3> GyroscopeBiasEstimator::GetGyroscopeBias() const {
...
bool GyroscopeBiasEstimator::IsCurrentEstimateValid() const {
    ...
    const auto current_gravity_dir = last_mean_filtered_accelerometer_value_.normalized();
    const auto gyro_bias_lowpass = gyroscope_bias_lowpass_filter_.GetFilteredData();

    const auto off_gravity_gyro_bias = gyro_bias_lowpass - current_gravity_dir * gyro_bias_lowpass.dot(current_gravity_dir);

    // Checks that the current bias estimate is not correlated with the
    // rotation computed from accelerometer.
    const auto gyro_from_accel = simulated_gyroscope_from_accelerometer_lowpass_filter_.GetFilteredData();
    const bool isGyroscopeBiasCorrelatedWithSimulatedGyro =
        (gyro_from_accel.norm() * kRatioBetweenGyroBiasAndAccel > (off_gravity_gyro_bias.norm() + kEpsilon));

    const bool hasEnoughSamples = current_accumulated_weights_gyroscope_bias_ > kMinSumOfWeightsGyroBiasThreshold;

    const bool areCountersStatic =
        gyroscope_static_counter_->IsRecentlyStatic() && accelerometer_static_counter_->IsRecentlyStatic();

    const bool isStatic = hasEnoughSamples && areCountersStatic && !isGyroscopeBiasCorrelatedWithSimulatedGyro;
    return isStatic;
}
