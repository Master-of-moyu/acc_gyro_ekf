...
void LowpassFilter::AddSample(const vector<3>& sample, uint64_t timestamp_ns) {
    AddWeightedSample(sample, timestamp_ns, 1.0);
}

void LowpassFilter::AddWeightedSample(const vector<3>& sample, uint64_t timestamp_ns, double weight) {
    if (!initialized_) {
    ...
    const double delta_s = static_cast<double>(timestamp_ns - timestamp_most_recent_update_ns_) * kSecondsFromNanoseconds;
